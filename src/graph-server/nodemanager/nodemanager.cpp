 ...
 *
 */
void NodeManager::init(std::string dshMachinesFile, std::string myPrIpFile,
                       Engine *_engine)
{
    ...
    printLog(500, "NodeManager is binding to %d", nodePort);
...
{
    // printLog(me.id, "Standalone value of %d", standAlone);
    if (standAlone)
        return;
    ...
    *((NodeMessage *)outMsg.data()) = nMsg;
    nodePublisher->send(outMsg);
    // printLog(me.id, "Sent outgoing message");

    // Keeps receiving BARRIER messages until heard from all (including self).
    remaining += allNodes.size();
    // printLog(me.id, "Got remaining of %d", remaining);
    zmq::message_t inMsg;
    while (remaining > 0)
    {
        // printLog(me.id, "Listening for incoming message");
        nodeSubscriber->recv(&inMsg);
        NodeMessage nMsg = *((NodeMessage *)inMsg.data());
        parseNodeMsg(nMsg);
        // printLog(me.id, "Got incoming message");
    }
 ...
 *
 */
void NodeManager::parseNodeConfig(const std::string dshMachinesFile)
{
